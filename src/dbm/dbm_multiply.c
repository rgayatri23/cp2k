...
static float *compute_rows_max_eps(const bool trans, const dbm_matrix_t *matrix,
  ...
  int *nblocks_per_row = (int*) dbm_calloc(nrows, sizeof(int));
  float *row_max_eps = (float*) dbm_malloc(nrows * sizeof(float));
...
static backend_context_t *backend_start(const dbm_matrix_t *matrix_c) {
  backend_context_t *ctx = (backend_context_t*) dbm_calloc(1, sizeof(backend_context_t));

...
static dbm_matrix_t *create_some_matrix(const int nrows, const int ncols,
  ...
  // Create distribution.
  int *row_dist = (int*) dbm_malloc(nrows * sizeof(int));
  int *col_dist = (int*) dbm_malloc(ncols * sizeof(int));
  ...
  // Create matrix.
  int *row_sizes = (int*) dbm_malloc(nrows * sizeof(int));
  int *col_sizes = (int*) dbm_malloc(ncols * sizeof(int));
  for (int i = 0; i < nrows; i++) {
...
#pragma omp for collapse(2)
    ...
    int *reserve_row = (int*) dbm_malloc(nblocks * sizeof(int));
    int *reserve_col = (int*) dbm_malloc(nblocks * sizeof(int));
    int iblock = 0;
#pragma omp for collapse(2)
    for (int row = 0; row < nrows; row++) {
      for (int col = 0; col < ncols; col++) {

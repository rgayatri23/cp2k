...
static void dbm_dist_1d_new(dbm_dist_1d_t *dist, const int length,
  ...
  dist->length = length;
  dist->index2coord = (int*) dbm_malloc(length * sizeof(int));
  ...
  dist->local_indicies = (int*) dbm_malloc(dist->nlocals * sizeof(int));
  int j = 0;
  for (int i = 0; i < length; i++) {
    if (coords[i] == dist->my_rank) {
...
void dbm_distribution_new(dbm_distribution_t **dist_out, const int fortran_comm,
  ...
  dbm_distribution_t *dist = (dbm_distribution_t*) dbm_calloc(1, sizeof(dbm_distribution_t));
  dist->ref_count = 1;

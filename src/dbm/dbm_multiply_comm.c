...
#pragma omp for
    for (int ipack = 0; ipack < npacks; ipack++) {
      plans_per_pack[ipack] = (plan_t*) dbm_malloc(nblks_per_pack[ipack] * sizeof(plan_t));
    }
...
static void postprocess_received_blocks(
  ...
  dbm_pack_block_t *blocks_tmp =
      (dbm_pack_block_t*) dbm_malloc(nblocks_recv * sizeof(dbm_pack_block_t));
...
static dbm_packed_matrix_t pack_matrix(const bool trans_matrix,
  ...
  packed.send_packs = (dbm_pack_t*) dbm_malloc(nsend_packs * sizeof(dbm_pack_t));
  ...
  for (int ipack = 0; ipack < nsend_packs; ipack++) {
    ...
    dbm_pack_block_t *blks_send =
        (dbm_pack_block_t*) dbm_malloc(nblks_send_per_pack[ipack] * sizeof(dbm_pack_block_t));
    double *data_send =
  ...
  packed.recv_pack.blocks =
      (dbm_pack_block_t*) dbm_malloc(packed.max_nblocks * sizeof(dbm_pack_block_t));
...
dbm_comm_iterator_t *dbm_comm_iterator_start(const bool transa,
  ...
  dbm_comm_iterator_t *iter = (dbm_comm_iterator_t*) dbm_malloc(sizeof(dbm_comm_iterator_t));

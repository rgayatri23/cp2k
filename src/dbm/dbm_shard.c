...
static void hashtable_init(dbm_shard_t *shard) {
  ...
  shard->hashtable = (int*) dbm_calloc(shard->hashtable_size, sizeof(int));
}
...
void dbm_shard_init(dbm_shard_t *shard) {
  ...
  shard->blocks = (dbm_block_t*) dbm_malloc(shard->nblocks_allocated * sizeof(dbm_block_t));
  hashtable_init(shard);
  shard->data_size = 0;
  shard->data_promised = 0;
  shard->data_allocated = INITIAL_DATA_ALLOCATED;
  shard->data = (double*) dbm_malloc(shard->data_allocated * sizeof(double));
...
void dbm_shard_copy(dbm_shard_t *shard_a, const dbm_shard_t *shard_b) {
  ...
  shard_a->blocks = (dbm_block_t*) dbm_malloc(shard_b->nblocks_allocated * sizeof(dbm_block_t));
  ...
  shard_a->hashtable = (int*) dbm_malloc(shard_b->hashtable_size * sizeof(int));
  memcpy(shard_a->hashtable, shard_b->hashtable,
         shard_b->hashtable_size * sizeof(int));

  free(shard_a->data);
  shard_a->data_allocated = shard_b->data_allocated;
  shard_a->data = (double*) dbm_malloc(shard_b->data_allocated * sizeof(double));
...
dbm_block_t *dbm_shard_promise_new_block(dbm_shard_t *shard, const int row,
  ...
  if (shard->nblocks_allocated < shard->nblocks + 1) {
    ...
    shard->blocks =
        (dbm_block_t*) dbm_realloc(shard->blocks, shard->nblocks_allocated * sizeof(dbm_block_t));
...
void dbm_shard_allocate_promised_blocks(dbm_shard_t *shard) {
  ...
  if (shard->data_promised > shard->data_allocated) {
    ...
    shard->data = (double*) dbm_realloc(shard->data, shard->data_allocated * sizeof(double));
  }

...
#if defined(__parallel)
  int *recvbuf = (int*) dbm_malloc(count * sizeof(int));
...
#if defined(__parallel)
  double *recvbuf = (double*) dbm_malloc(count * sizeof(double));
...
void dbm_mpi_sum_int64(int64_t *values, const int count,
                       const dbm_mpi_comm_t comm) {
#if defined(__parallel)
  int64_t *recvbuf = (int64_t*) dbm_malloc(count * sizeof(int64_t));

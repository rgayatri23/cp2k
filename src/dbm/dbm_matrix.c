...
void dbm_create(dbm_matrix_t **matrix_out, dbm_distribution_t *dist,
  ...
  dbm_matrix_t *matrix = (dbm_matrix_t*) dbm_calloc(1, sizeof(dbm_matrix_t));
  ...
  matrix->name = (char*) dbm_malloc(size);
  ...
  matrix->row_sizes = (int*) dbm_malloc(size);
  memcpy(matrix->row_sizes, row_sizes, size);

  size = ncols * sizeof(int);
  matrix->col_sizes = (int*) dbm_malloc(size);
  memcpy(matrix->col_sizes, col_sizes, size);

  matrix->shards = (dbm_shard_t*) dbm_malloc(dbm_get_num_shards(matrix) * sizeof(dbm_shard_t));
...
void dbm_redistribute(const dbm_matrix_t *matrix, dbm_matrix_t *redist) {
  ...
  double *data_send = (double*) dbm_malloc(total_send_count * sizeof(double));
  double *data_recv = (double*) dbm_malloc(total_recv_count * sizeof(double));
...
void dbm_iterator_start(dbm_iterator_t **iter_out, const dbm_matrix_t *matrix) {
  ...
  dbm_iterator_t *iter = (dbm_iterator_t*) dbm_malloc(sizeof(dbm_iterator_t));
  iter->matrix = matrix;
  iter->next_block = 0;
